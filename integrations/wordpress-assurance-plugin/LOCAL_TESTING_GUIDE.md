...
services:
  wordpress:
    ...
    volumes:
      - ./plugin/encypher-assurance:/var/www/html/wp-content/plugins/encypher-assurance
...
1. Copy plugin folder:
   ```powershell
   copy -Recurse plugin\encypher-assurance "C:\Users\eriks\Local Sites\test-publisher\app\public\wp-content\plugins\"
   ```
...
**Test sign endpoint:**
```powershell
curl -X POST http://localhost:8080/wp-json/encypher-assurance/v1/sign `
...
```powershell
curl -X POST http://localhost:8080/wp-json/encypher-assurance/v1/verify `
  -H "X-WP-Nonce: [GET_FROM_BROWSER]" `
  -H "Content-Type: application/json" `
  -d '{"post_id": 1}'
```
...
```powershell
curl http://localhost:8080/wp-json/encypher-assurance/v1/status?post_id=1 `
  -H "X-WP-Nonce: [GET_FROM_BROWSER]"
```
