...
class Verification
{
    ...
    public function append_verification_badge(string $content): string
    {
        ...
        if ($auto_verify && (! is_array($verification) || ! array_key_exists('is_valid', $verification))) {
            $request = new \WP_REST_Request('POST', '/encypher-assurance/v1/verify');
        ...
        ob_start();
        ?>
        <section class="encypher-assurance-verification" aria-label="<?php echo esc_attr__('Encypher verification status', 'encypher-assurance'); ?>">
            <h2 class="verification-title">
                <?php
                if ($tampered) {
                    esc_html_e('Potential tampering detected', 'encypher-assurance');
                } elseif ($is_valid) {
                    esc_html_e('Content authenticity verified', 'encypher-assurance');
                } else {
                    esc_html_e('Signature not verified', 'encypher-assurance');
                }
                ?>
            </h2>
            <ul class="verification-meta">
                <?php if ($document_id) : ?>
                    <li><strong><?php esc_html_e('Document ID:', 'encypher-assurance'); ?></strong> <?php echo esc_html($document_id); ?></li>
                <?php endif; ?>
                <?php if ($status) : ?>
                    <li><strong><?php esc_html_e('Status:', 'encypher-assurance'); ?></strong> <?php echo esc_html($status); ?></li>
                <?php endif; ?>
                <?php if ($organization) : ?>
                    <li><strong><?php esc_html_e('Signed by:', 'encypher-assurance'); ?></strong> <?php echo esc_html($organization); ?><?php if ($signer_id) : ?> (<?php echo esc_html($signer_id); ?>)<?php endif; ?></li>
                <?php elseif ($signer_id) : ?>
                    <li><strong><?php esc_html_e('Signer ID:', 'encypher-assurance'); ?></strong> <?php echo esc_html($signer_id); ?></li>
                <?php endif; ?>
                <?php if ($timestamp) : ?>
                    <li><strong><?php esc_html_e('Signed at:', 'encypher-assurance'); ?></strong> <?php echo esc_html($timestamp); ?></li>
                <?php endif; ?>
                <?php if ($last_signed) : ?>
                    <li><strong><?php esc_html_e('Last signed in WordPress:', 'encypher-assurance'); ?></strong> <?php echo esc_html($last_signed); ?></li>
                <?php endif; ?>
                <?php if ($total_sentences) : ?>
                    <li><strong><?php esc_html_e('Sentences protected:', 'encypher-assurance'); ?></strong> <?php echo esc_html((string) $total_sentences); ?></li>
                <?php endif; ?>
            </ul>
            ...
            <?php if ($manifest) : ?>
                <details class="verification-details">
                    <summary><?php esc_html_e('View manifest JSON', 'encypher-assurance'); ?></summary>
