...
| GHE-009 | Legacy Mobiledoc/Lexical migration output | structure drift | signer operates on rendered HTML safely | ⚠️ pending local replay |
| GHE-010 | Round-trip Ghost reserialization after update | verification regressions | extraction(in) == extraction(out, stripping markers) | ⚠️ pending local replay |
| GHE-011 | Unicode graphemes (emoji, ZWJ) | split/index mismatch | codepoint-safe insertion and verification | ⚠️ partial (unit only) |
| GHE-012 | Malformed but accepted HTML | parser divergence | no crashes; conservative fallback | ⚠️ pending |
