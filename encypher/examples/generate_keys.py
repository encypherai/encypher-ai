...
def generate_and_print_keys():
    ...
    console.print(
        Panel("[bold cyan]EncypherAI Key Pair Generator[/bold cyan]", expand=False)
    )
    ...
    console.print(
        "   [yellow]Store this key securely and NEVER commit it to version control.[/yellow]"
    )
    console.print(
        Syntax(private_pem.decode("utf-8"), "pem", theme="default", line_numbers=False)
    )

    console.print("\n[bold]2. Public Key (PEM Format):[/bold]")
    console.print(
        "   Share this key with systems that need to verify your signed data."
    )
    console.print(
        Syntax(public_pem.decode("utf-8"), "pem", theme="default", line_numbers=False)
    )
    ...
    console.print(
        "   - Add the entire Private Key PEM block (including -----BEGIN/END----- lines)"
    )
    ...
    console.print(
        "     (Ensure your .env loading mechanism handles multiline variables correctly, or store without newlines)"
    )
    console.print("2. [bold]Load the Private Key in your app:[/bold]")
    console.print(
        "   - Use `os.getenv('ENCYPHER_PRIVATE_KEY')` and `serialization.load_pem_private_key`."
    )
    ...
    console.print(
        f"   - When embedding metadata, ensure your metadata dictionary includes `{example_dict_str}`."
    )
    console.print(
        "   - Configure your `public_key_resolver` function to return the Public Key PEM"
    )
