...
Verifying embedded metadata is a critical step to ensure the authenticity and integrity of text content. The Encypher SDK provides a robust verification process that handles both legacy formats and the latest C2PA v2.3 standard.
...
3.  **Signature Validation**: It verifies the cryptographic signature using the key provided by the `public_key_resolver`.
4.  **Deep Manifest Validation (for C2PA v2.3)**: If a C2PA v2.3 manifest is detected, it performs additional, deeper validation:
...
The `verify_metadata` method requires a `public_key_resolver`. This is a callable function that you provide, which is responsible for supplying the correct public key for a given `signer_id`.
...
## Multiple Wrapper Strictness

C2PA v2.3 defines a failure code for texts that contain more than one valid `C2PATextManifestWrapper` (`manifest.text.multipleWrappers`). In `encypher-ai`, verification is strict-by-default:

- If more than one valid wrapper is detected, `UnicodeMetadata.verify_metadata()` returns `False`.
