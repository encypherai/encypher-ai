# Encypher Privacy Policy

**Last Updated:** October 28, 2025
**Effective Date:** October 28, 2025

## 1. Introduction

Encypher ("we", "us", or "our") is committed to protecting your privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use the Encypher Enterprise API and related services (collectively, the "Service").

Please read this Privacy Policy carefully. By using the Service, you agree to the collection and use of information in accordance with this policy.

## 2. Information We Collect

### 2.1 Information You Provide

**Account Information:**
- Organization name
- Organization type (publisher, legal/finance, AI lab, enterprise)
- Email address
- Billing information (for paid tiers)
- Contact name and phone number (optional)

**Content Data:**
- Text content you sign using the Service
- Document titles, URLs, and metadata
- Publication dates and author information
- Sentence-level text segments

**Certificate Information:**
- SSL.com certificate orders and status
- Public keys and certificates (PEM format)
- Certificate expiration dates

### 2.2 Information We Collect Automatically

**API Usage Data:**
- API endpoints accessed
- Request timestamps
- Response status codes
- Request/response sizes
- API key used (hashed)

**Technical Information:**
- IP addresses
- User agent strings
- Browser type and version
- Operating system
- Device identifiers

**Performance Data:**
- API latency metrics
- Error rates
- Service availability

### 2.3 Information from Third Parties

**SSL.com:**
- Certificate validation status
- Certificate issuance confirmations
- Certificate revocation status

**Payment Processors:**
- Payment confirmation
- Billing status
- Transaction IDs (we do not store credit card numbers)

## 3. How We Use Your Information

### 3.1 To Provide the Service

- Process content signing requests
- Generate C2PA manifests and signatures
- Store verification records
- Manage SSL.com certificates
- Provide verification endpoints

### 3.2 To Maintain and Improve the Service

- Monitor service performance
- Identify and fix bugs
- Analyze usage patterns
- Develop new features
- Optimize infrastructure

### 3.3 To Communicate with You

- Send service announcements
- Respond to support requests
- Send billing notifications
- Notify about certificate expiration
- Send security alerts

### 3.4 To Ensure Security and Prevent Fraud

- Detect and prevent abuse
- Enforce rate limits and quotas
- Investigate security incidents
- Comply with legal obligations
- Protect our rights and property

### 3.5 For Legal Compliance

- Respond to legal requests
- Enforce our Terms of Service
- Comply with applicable laws and regulations
- Protect against legal liability

## 4. How We Share Your Information

### 4.1 We Do Not Sell Your Information

We do not sell, rent, or trade your personal information to third parties for their marketing purposes.

### 4.2 Service Providers

We share information with third-party service providers who perform services on our behalf:

**SSL.com (Certificate Authority):**
- Organization name and email
- Certificate signing requests
- Public keys

**Railway (Hosting Provider):**
- All data stored in our database
- Application logs
- Performance metrics

**Payment Processors (Stripe):**
- Billing information
- Payment history
- Organization details

**Email Service (SendGrid):**
- Email addresses
- Email content (transactional only)

All service providers are contractually obligated to protect your information and use it only for the purposes we specify.

### 4.3 Public Verification Records

The following information is publicly accessible through our verification endpoints:
- Document titles and URLs
- Publication dates
- Publisher names (organization names)
- Sentence-level hashes (not the actual text)
- C2PA manifests and signatures
- Certificate information

This information is necessary to enable independent verification of signed content.

### 4.4 Legal Requirements

We may disclose your information if required by law or in response to:
- Court orders or subpoenas
- Government investigations
- Legal processes
- Protection of our rights or safety
- Enforcement of our Terms of Service

### 4.5 Business Transfers

If Encypher is involved in a merger, acquisition, or sale of assets, your information may be transferred. We will notify you before your information is transferred and becomes subject to a different privacy policy.

## 5. Data Retention

### 5.1 Active Accounts

For active accounts, we retain:
- Account information: Until account deletion
- Content metadata: Indefinitely (required for verification)
- API usage logs: 2 years
- Audit logs: 2 years

### 5.2 Deleted Accounts

When you delete your account:
- Account information: Deleted after 90 days
- Private data: Deleted after 90 days
- Verification records: Retained for 7 years (legal compliance)
- Public verification data: Retained indefinitely (necessary for independent verification)

### 5.3 Legal Holds

We may retain information longer if required by law or legal proceedings.

## 6. Data Security

### 6.1 Security Measures

We implement industry-standard security measures:

**Encryption:**
- TLS 1.3 for data in transit
- AES-256 encryption for data at rest
- Encrypted private key storage

**Access Controls:**
- API key authentication
- Role-based access control
- Multi-factor authentication for admin accounts
- Regular access audits

**Infrastructure Security:**
- Isolated database instances
- Firewall protection
- DDoS mitigation
- Regular security updates

**Monitoring:**
- 24/7 security monitoring
- Intrusion detection systems
- Automated threat detection
- Incident response procedures

### 6.2 Your Responsibilities

You are responsible for:
- Keeping your API keys confidential
- Using strong passwords
- Reporting security incidents promptly
- Complying with security best practices

### 6.3 No Absolute Security

While we implement strong security measures, no system is 100% secure. We cannot guarantee absolute security of your information.

## 7. Your Privacy Rights

### 7.1 Access and Portability

You have the right to:
- Access your personal information
- Request a copy of your data
- Export your data in JSON format
- Receive data within 30 days

**How to exercise:** Email privacy@encypher.com

### 7.2 Correction and Update

You have the right to:
- Correct inaccurate information
- Update your account details
- Modify organization information

**How to exercise:** Log into your account or email privacy@encypher.com

### 7.3 Deletion

You have the right to:
- Delete your account
- Request deletion of your personal information
- Receive confirmation of deletion

**Limitations:**
- Verification records retained for 7 years (legal compliance)
- Public verification data retained indefinitely (necessary for service)
- Information required by law cannot be deleted

**How to exercise:** Email privacy@encypher.com

### 7.4 Objection and Restriction

You have the right to:
- Object to certain processing activities
- Restrict processing of your information
- Opt-out of marketing communications

**How to exercise:** Email privacy@encypher.com

### 7.5 Withdraw Consent

Where we rely on consent, you may withdraw it at any time. This does not affect the lawfulness of processing before withdrawal.

### 7.6 Complaint

You have the right to lodge a complaint with a data protection authority in your jurisdiction.

## 8. International Data Transfers

### 8.1 Data Location

Our primary data centers are located in the United States. By using the Service, you consent to the transfer of your information to the United States.

### 8.2 EU-U.S. Data Transfers

For users in the European Economic Area (EEA), United Kingdom, or Switzerland:
- We comply with applicable data transfer mechanisms
- We use Standard Contractual Clauses (SCCs) approved by the European Commission
- We implement additional safeguards as required

### 8.3 Multi-Region Support (Enterprise Tier)

Enterprise customers may request data storage in specific regions:
- EU region (Frankfurt, Germany)
- Asia-Pacific region (Singapore)

Contact sales@encypher.com for multi-region options.

## 9. Children's Privacy

The Service is not intended for individuals under 18 years of age. We do not knowingly collect personal information from children. If you believe we have collected information from a child, contact us immediately at privacy@encypher.com.

## 10. California Privacy Rights (CCPA)

If you are a California resident, you have additional rights under the California Consumer Privacy Act (CCPA):

### 10.1 Right to Know

You have the right to request:
- Categories of personal information collected
- Sources of personal information
- Business purpose for collection
- Categories of third parties we share with
- Specific pieces of personal information

### 10.2 Right to Delete

You have the right to request deletion of your personal information, subject to certain exceptions.

### 10.3 Right to Opt-Out

We do not sell personal information. If our practices change, we will update this policy and provide an opt-out mechanism.

### 10.4 Right to Non-Discrimination

We will not discriminate against you for exercising your CCPA rights.

### 10.5 Authorized Agent

You may designate an authorized agent to make requests on your behalf. The agent must provide proof of authorization.

### 10.6 How to Exercise CCPA Rights

Email privacy@encypher.com with:
- Your name and email address
- Description of your request
- Proof of California residency (if applicable)

We will respond within 45 days.

## 11. European Privacy Rights (GDPR)

If you are in the European Economic Area (EEA), United Kingdom, or Switzerland, you have rights under the General Data Protection Regulation (GDPR):

### 11.1 Legal Basis for Processing

We process your information based on:
- **Contract**: To provide the Service you requested
- **Legitimate Interests**: To improve the Service, prevent fraud, ensure security
- **Consent**: For marketing communications (where required)
- **Legal Obligation**: To comply with applicable laws

### 11.2 Data Controller

Encypher is the data controller for your personal information.

### 11.3 Data Protection Officer

For GDPR inquiries, contact our Data Protection Officer:
Email: dpo@encypher.com

### 11.4 GDPR Rights

You have the right to:
- Access your personal data
- Rectify inaccurate data
- Erase your data ("right to be forgotten")
- Restrict processing
- Data portability
- Object to processing
- Withdraw consent
- Lodge a complaint with a supervisory authority

### 11.5 How to Exercise GDPR Rights

Email privacy@encypher.com or dpo@encypher.com

We will respond within 30 days.

## 12. Cookies and Tracking Technologies

### 12.1 Cookies We Use

**Essential Cookies:**
- Session management
- Authentication
- Security

**Analytics Cookies:**
- Usage statistics
- Performance monitoring
- Error tracking

**Preference Cookies:**
- Language preferences
- Display settings

### 12.2 Third-Party Cookies

We may use third-party analytics services (e.g., Google Analytics) that use cookies. These services have their own privacy policies.

### 12.3 Cookie Control

You can control cookies through your browser settings. Disabling essential cookies may affect Service functionality.

## 13. Do Not Track Signals

Our Service does not respond to Do Not Track (DNT) signals. We may track your activity across our Service for analytics and security purposes.

## 14. Changes to This Privacy Policy

### 14.1 Notification of Changes

We may update this Privacy Policy from time to time. We will notify you of material changes by:
- Email notification (to your registered email)
- Prominent notice on our website
- In-app notification

### 14.2 Effective Date

Changes become effective 30 days after notification, unless you object by discontinuing use of the Service.

### 14.3 Review

We encourage you to review this Privacy Policy periodically.

## 15. Contact Us

For questions, concerns, or requests regarding this Privacy Policy or your personal information:

**Privacy Inquiries:**
Email: privacy@encypher.com

**Data Protection Officer (GDPR):**
Email: dpo@encypher.com

**General Contact:**
Email: legal@encypher.com
Website: https://encypher.com

**Mailing Address:**
Encypher
[Your Physical Address]
[City, State, ZIP]
United States

## 16. Definitions

**Personal Information**: Information that identifies, relates to, or could reasonably be linked with you or your household.

**Processing**: Any operation performed on personal information, including collection, storage, use, disclosure, and deletion.

**Service**: The Encypher Enterprise API and all related services, websites, and applications.

**Third Party**: Any individual or entity that is not you or Encypher.

---

**By using the Encypher Service, you acknowledge that you have read, understood, and agree to this Privacy Policy.**
