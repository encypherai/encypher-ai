...
async def list_organizations(
    ...
    return {
        "success": True,
        "data": [OrganizationResponse.model_validate(org).model_dump() for org in orgs],
        "error": None
    }
...
async def list_invitations(
    ...
    return {
        "success": True,
        "data": [InvitationResponse.model_validate(inv).model_dump() for inv in invitations],
        "error": None
    }
...
async def resend_invitation(
    ...
    try:
        org_service = OrganizationService(db)
        invitation = org_service.resend_invitation(
            org_id=org_id,
            invitation_id=invitation_id,
            actor_user_id=user_id
        )
...
async def accept_invitation_new_user(
    ...
    try:
        org_service = OrganizationService(db)
        user, member = org_service.accept_invitation_new_user(
            token=token,
            name=data.name,
