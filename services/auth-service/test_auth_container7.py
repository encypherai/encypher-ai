...
# Wait, the user `test_5fc8a3@encypher.com` has `totp_enabled` but hasn't actually configured a secret!
...
cur = conn.cursor()
cur.execute("SELECT totp_secret FROM users WHERE email='test_5fc8a3@encypher.com'")
row = cur.fetchone()
