...
email = f"test_{uuid.uuid4().hex[:6]}@encypherai.com"
...
with open("verify_email.sh", "w") as f:
    f.write(f"docker exec -i encypher-postgres psql -U encypher -d encypher_auth -c \"UPDATE users SET email_verified = true WHERE email = '{email}';\"\n")

# Enable TOTP directly in DB
with open("enable_totp.sh", "w") as f:
    f.write(f"docker exec -i encypher-postgres psql -U encypher -d encypher_auth -c \"UPDATE users SET totp_enabled = true WHERE email = '{email}';\"\n")
