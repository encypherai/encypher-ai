...
This module is a local copy of the enterprise implementation so the
verification-service can validate embedded C2PA certificate chains without
depending on enterprise-api code.
"""
...
def set_trust_anchors_pem(pem_data: str) -> int:
    ...
    logger.info("Loaded %s C2PA trust anchors", len(_trust_anchors))
...
def _verify_cert_signature(*, subject: Certificate, issuer: Certificate) -> bool:
    ...
    try:
        ...
        if isinstance(issuer_pk, EllipticCurvePublicKey):
            ...
            if algo is None:
                return False
            issuer_pk.verify(subject.signature, subject.tbs_certificate_bytes, ec.ECDSA(algo))
            return True
...
def validate_certificate_chain(
    ...
    try:
        ...
        return False, f"Invalid certificate format: {exc}", None
