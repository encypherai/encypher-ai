...
# 1. Create a user
email = f"test_mfa_{uuid.uuid4().hex[:6]}@encypher.com"
password = "Password123!"

req = urllib.request.Request(
    "http://localhost:8001/api/v1/auth/signup",
    data=json.dumps({"email": email, "password": password, "name": "MFA Test User"}).encode("utf-8"),
...
# 3. Setup MFA
req = urllib.request.Request(
    "http://localhost:8001/api/v1/auth/mfa/totp/setup", data=b"", headers={"Authorization": f"Bearer {token}", "Content-Length": "0"}
)
