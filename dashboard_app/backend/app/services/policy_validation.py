...
async def get_policy_schemas(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[PolicySchema]:
    """
    Get policy schemas.
    """
...
async def get_validation_results(
    db: AsyncSession, filters: Optional[PolicyValidationFilters] = None, skip: int = 0, limit: int = 100
    ...
    result = await db.execute(stmt.order_by(PolicyValidationResult.validation_time.desc()).offset(skip).limit(limit))
    return result.scalars().all()
...
async def get_validation_stats(db: AsyncSession, filters: Optional[PolicyValidationFilters] = None) -> PolicyValidationStats:
    ...
    department_stats_stmt = select(PolicyValidationResult.department, func.count(PolicyValidationResult.id)).where(
        PolicyValidationResult.department.isnot(None)
    )
...
async def import_validation_results_from_csv(db: AsyncSession, csv_file_path: str) -> int:
    ...
    if not default_schema:
        default_schema = PolicySchema(
            name="Default Policy", description="Default policy schema created during CSV import", schema_definition={"rules": []}
        )
