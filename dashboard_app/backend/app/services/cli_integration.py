...
async def run_policy_validator_cli(
    ...
    cli_path = os.path.abspath(os.path.join(
        os.path.dirname(__file__), 
        "..", "..", "..", "..", 
        "policy_validator_cli", "app", "main.py"
    ))
...
async def import_cli_results_to_db(
    ...
    policy_validation_file: Optional[str] = None
) -> Dict[str, Any]:
    """
...
async def run_scheduled_scan(
    ...
    async def _execute_scan_logic(session: AsyncSession):
        ...
        try:
            audit_success, audit_file_path, audit_error = await run_audit_log_cli(
                input_path=input_path,
                output_format="csv",
                verbose=True
            )
            ...
            if audit_success:
                ...
                if audit_file_path: audit_log_temp_dir_local = os.path.dirname(audit_file_path)
            else:
            ...
            if policy_success:
                ...
                if policy_val_file_path: policy_validation_temp_dir_local = os.path.dirname(policy_val_file_path)
            else:
