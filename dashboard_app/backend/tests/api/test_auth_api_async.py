...
async def test_login_for_access_token_success(client: AsyncClient, db: AsyncSession) -> None:
    ...
    await create_user(db=db, username=user_in.username, email=user_in.email, password=user_in.password, full_name=user_in.full_name, is_superuser=user_in.is_superuser if hasattr(user_in, 'is_superuser') else False)
...
async def test_login_for_access_token_incorrect_password(client: AsyncClient, db: AsyncSession) -> None:
    ...
    user_in = UserCreate(
        ...
        password=test_password,
        full_name="Login WrongPass User"
    )
    await create_user(db=db, username=user_in.username, email=user_in.email, password=user_in.password, full_name=user_in.full_name, is_superuser=user_in.is_superuser if hasattr(user_in, 'is_superuser') else False)
...
async def test_read_users_me_success(client: AsyncClient, db: AsyncSession) -> None:
    ...
    created_user = await create_user(db=db, username=user_in.username, email=user_in.email, password=user_in.password, full_name=user_in.full_name, is_superuser=user_in.is_superuser if hasattr(user_in, 'is_superuser') else False)
