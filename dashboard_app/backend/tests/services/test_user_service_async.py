...
async def test_create_user(db: AsyncSession) -> None:
    ...
    user_in = UserCreate(username="testuser_create", email="test_create@example.com", password="testpassword123", full_name="Test User Create")
    created_user = await create_user(db, username=user_in.username, email=user_in.email, password=user_in.password, full_name=user_in.full_name)
...
async def test_get_user_by_username_existing(db: AsyncSession) -> None:
    ...
    user_in = UserCreate(username="testuser_get", email="test_get@example.com", password="securepassword", full_name="Test User Get")
    await create_user(db, username=user_in.username, email=user_in.email, password=user_in.password, full_name=user_in.full_name)  # Create user first
...
async def test_authenticate_user_correct_credentials(db: AsyncSession) -> None:
    ...
    user_in = UserCreate(username="auth_user_correct", email="auth_correct@example.com", password="authpass123", full_name="Auth User Correct")
    await create_user(db, username=user_in.username, email=user_in.email, password=user_in.password, full_name=user_in.full_name)
...
async def test_authenticate_user_incorrect_password(db: AsyncSession) -> None:
    ...
    user_in = UserCreate(
        username="auth_user_incorrect_pass", email="auth_incorrect_pass@example.com", password="correctpass", full_name="Auth User Incorrect Pass"
    )
    await create_user(db, username=user_in.username, email=user_in.email, password=user_in.password, full_name=user_in.full_name)
