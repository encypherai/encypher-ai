...
try:
    ...
    for signer in signers:
        ...
        ea = Encypher(private_key_path=str(private_key_path), public_key_path=str(public_key_path), signer_id=signer_id, verbose=True)
...
DOCX_BASE64_END"""
        ...
        text_with_metadata = ea.embed_metadata(
            text=text_wrapper, custom_metadata=metadata, model_id=model_id, metadata_format="basic", timestamp=timestamp
        )
...
PDF_BASE64_END"""
    ...
    def create_text_file_with_metadata(
        ...
        # Embed metadata
        text_with_metadata = ea.embed_metadata(
            text=content, custom_metadata=metadata, model_id=model_id, metadata_format="basic", timestamp=timestamp
        )
        ...
        if tamper:
            ...
            if len(lines) > 2:
                ...
                text_with_metadata = text_with_metadata.replace("This document", "This document [THIS TEXT WAS MODIFIED AFTER SIGNING]")
    ...
    with Progress(TextColumn("[bold blue]{task.description}"), BarColumn(), TaskProgressColumn(), console=console) as progress:
        ...
        categories = [
            ...
            {"name": "Compliance Documents", "models": ["compliance-check-v1", "audit-report-v1"], "signers": ["compliance-dept"]},
            {"name": "External Documents", "models": ["external-report-v1"], "signers": ["untrusted-source"]},
        ]
...
""")
    ...
    console.print(
        "uv run -- python -m app.main --target dashboard_app/examples/files --trusted-signers dashboard_app/examples/files/keys --output dashboard_example_report.csv"
    )
