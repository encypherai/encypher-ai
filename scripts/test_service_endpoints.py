...
def get_auth_token():
    ...
    # Signup
    try:
        resp = requests.post(f"{BASE_URLS['auth']}/api/v1/auth/signup", json={"email": email, "password": password, "name": "Test User"}, timeout=10)
        ...
        # Login
        resp = requests.post(f"{BASE_URLS['auth']}/api/v1/auth/login", json={"email": email, "password": password}, timeout=10)
...
def test_key_service(token: str):
    ...
    if api_key:
        try:
            resp = requests.post(f"{BASE_URLS['key']}/api/v1/keys/verify", json={"key": api_key}, timeout=5)
            is_valid = resp.status_code == 200 and resp.json().get("valid", False)
            results.record(
                "POST /api/v1/keys/verify",
                is_valid,
                f"Status {resp.status_code}, valid={resp.json().get('valid') if resp.status_code == 200 else 'N/A'}",
            )
        except Exception as e:
...
def test_user_service(token: str, user_id: str):
    ...
    # Update profile
    try:
        resp = requests.put(f"{BASE_URLS['user']}/api/v1/profile", json={"display_name": "Test User", "bio": "Testing"}, headers=headers, timeout=5)
...
def test_analytics_service(token: str):
    ...
    try:
        resp = requests.post(
            f"{BASE_URLS['analytics']}/api/v1/analytics/track", json={"metric_type": "api_call", "service_name": "test"}, headers=headers, timeout=5
        )
...
def test_encoding_service(token: str):
    ...
    try:
        resp = requests.post(
            f"{BASE_URLS['encoding']}/api/v1/encode", json={"content": "Test content for encoding", "format": "text"}, headers=headers, timeout=10
        )
...
def test_verification_service(token: str):
    ...
    # Verify content
    try:
        resp = requests.post(f"{BASE_URLS['verification']}/api/v1/verify", json={"content": "Test content to verify"}, headers=headers, timeout=10)
