...
pytestmark = pytest.mark.skip(reason="Legacy embedding API - current architecture uses C2PA signing via /api/v1/sign")
...
class TestEncodeWithEmbeddingsEndpoint:
    ...
    def test_encode_with_embeddings_basic(self, mock_embedding_service, mock_merkle_service):
        ...
        mock_merkle_service.encode_document = AsyncMock(return_value={"sentence": mock_root})
        ...
        mock_embeddings = [
            EmbeddingReference(
                ref_id=0xA3F9C2E1, signature="8k3mP9xQ12345678", leaf_hash="hash1", leaf_index=0, text_content="Sentence one.", document_id="doc_001"
            ),
            EmbeddingReference(
                ref_id=0xB4A8D3F2, signature="9m4nQ0yR12345678", leaf_hash="hash2", leaf_index=1, text_content="Sentence two.", document_id="doc_001"
            ),
        ]
        ...
        response = client.post(
            ...
            json={"document_id": "doc_001", "text": "Sentence one. Sentence two.", "segmentation_level": "sentence"},
        )
...
class TestBatchVerifyEndpoint:
    ...
    def test_batch_verify_mixed_results(self, mock_embedding_service):
        ...
        response = client.post(
            ...
            json={"references": [{"ref_id": "a3f9c2e1", "signature": "8k3mP9xQ"}, {"ref_id": "invalid1", "signature": "00000000"}]},
        )
