...
class TestWebSocketBasicFlow:
    ...
    async def test_websocket_connection(self, demo_api_key):
        ...
        uri = f"ws://localhost:8000/api/v1/sign/stream?api_key={demo_api_key}"
    ...
    async def test_chunk_processing(self, demo_api_key):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            # Send chunk
            await websocket.send(json.dumps({"type": "chunk", "content": "This is a test chunk. ", "chunk_id": "chunk_001"}))
    ...
    async def test_stream_finalization(self, demo_api_key):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            for i, chunk in enumerate(chunks):
                await websocket.send(json.dumps({"type": "chunk", "content": chunk, "chunk_id": f"chunk_{i:03d}"}))
...
class TestWebSocketAuthentication:
    ...
    async def test_missing_api_key(self):
        ...
        uri = "ws://localhost:8000/api/v1/sign/stream"
    ...
    async def test_invalid_api_key(self):
        ...
        uri = "ws://localhost:8000/api/v1/sign/stream?api_key=invalid_key"
...
class TestRateLimiting:
    ...
    async def test_chunk_rate_limiting(self, demo_api_key):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            rate_limited = False
            for i in range(100):
                await websocket.send(json.dumps({"type": "chunk", "content": f"Chunk {i}. ", "chunk_id": f"chunk_{i:03d}"}))
...
class TestSessionRecovery:
    ...
    async def test_session_recovery(self, demo_api_key, setup_redis):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            # Send a few chunks
            for i in range(3):
                await websocket.send(json.dumps({"type": "chunk", "content": f"Chunk {i}. "}))
                await websocket.recv()
        ...
        async with websockets.connect(uri) as websocket:
            ...
            await websocket.send(json.dumps({"type": "recover_session", "session_id": session_id}))
...
class TestErrorHandling:
    ...
    async def test_invalid_message_type(self, demo_api_key):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            await websocket.send(json.dumps({"type": "invalid_type", "content": "test"}))
    ...
    async def test_multiple_concurrent_connections(self, demo_api_key):
        ...
        async def create_and_use_connection(conn_id: int):
            ...
            async with websockets.connect(uri) as websocket:
                ...
                # Send a few chunks
                for i in range(5):
                    await websocket.send(json.dumps({"type": "chunk", "content": f"Connection {conn_id}, Chunk {i}. "}))
                    await websocket.recv()
    ...
    async def test_throughput(self, demo_api_key):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            for i in range(num_chunks):
                await websocket.send(json.dumps({"type": "chunk", "content": f"Performance test chunk {i}. "}))
