...
class TestLoadSimulation:
    def test_sign_endpoint_performance_cpu(self):
        ...
        # Apply patches
        with (
            patch.object(ksc_module, "key_service_client", mock_key_client),
            ...
            for _ in range(10):
                resp = client.post(
                    "/api/v1/sign", json={"text": "Warmup content " * 5, "title": "Warmup"}, headers={"Authorization": "Bearer test_key"}
                )
            ...
            for i in range(100):
                t0 = time.time()
                resp = client.post("/api/v1/sign", json=payload, headers={"Authorization": "Bearer test_key"})
                t1 = time.time()
                assert resp.status_code == 200
