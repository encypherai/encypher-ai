...
class TestC2PASchemaAPI:
    ...
    async def test_create_schema(self, client: AsyncClient, auth_headers: dict):
        ...
        schema_data = {
            ...
            "is_public": False,
            "json_schema": {"type": "object", "properties": {"field1": {"type": "string"}, "field2": {"type": "number"}}, "required": ["field1"]},
        }

        response = await client.post("/api/v1/enterprise/c2pa/schemas", json=schema_data, headers=auth_headers)
    ...
    async def test_create_schema_duplicate(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        schema_data = {"name": "Duplicate Test Schema", "label": unique_label, "version": "1.0", "json_schema": {"type": "object"}}
    ...
    async def test_list_schemas(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        response = await client.get("/api/v1/enterprise/c2pa/schemas", headers=auth_headers)
    ...
    async def test_list_schemas_with_is_public_filter(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        response = await client.get("/api/v1/enterprise/c2pa/schemas?is_public=true", headers=auth_headers)
    ...
    async def test_get_schema(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        response = await client.get(f"/api/v1/enterprise/c2pa/schemas/{schema.id}", headers=auth_headers)
    ...
    async def test_get_schema_not_found(self, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.get(f"/api/v1/enterprise/c2pa/schemas/{fake_id}", headers=auth_headers)
    ...
    async def test_update_schema(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        response = await client.put(f"/api/v1/enterprise/c2pa/schemas/{schema.id}", json=update_data, headers=auth_headers)
    ...
    async def test_delete_schema(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        response = await client.delete(f"/api/v1/enterprise/c2pa/schemas/{schema.id}", headers=auth_headers)
...
class TestC2PAValidationAPI:
    ...
    async def test_validate_assertion_valid(self, client: AsyncClient, auth_headers: dict):
        ...
        assertion_data = {"label": "c2pa.location.v1", "data": {"latitude": 37.7749, "longitude": -122.4194}}

        response = await client.post("/api/v1/enterprise/c2pa/validate", json=assertion_data, headers=auth_headers)
...
class TestC2PATemplateAPI:
    ...
    async def test_create_template(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        schema = C2PASchema(
            name="Template Test Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_demo"
        )
        db.add(schema)
        await db.commit()
        await db.refresh(schema)
        ...
        template_data = {
            ...
            "category": "news",
            "is_public": False,
            "template_data": {"label": "c2pa.location.v1", "description": "Story location", "optional": True},
        }

        response = await client.post("/api/v1/enterprise/c2pa/templates", json=template_data, headers=auth_headers)
    ...
    async def test_list_templates(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        schema = C2PASchema(
            name="List Template Test Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_demo"
        )
        db.add(schema)
        await db.commit()
        await db.refresh(schema)
        ...
        for i in range(3):
            template = C2PAAssertionTemplate(
                name=f"Template {uuid.uuid4().hex[:8]}", schema_id=schema.id, template_data={}, category="news", organization_id="org_demo"
            )
            db.add(template)
        await db.commit()

        response = await client.get("/api/v1/enterprise/c2pa/templates", headers=auth_headers)
    ...
    async def test_list_templates_with_category_filter(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        schema = C2PASchema(
            name="Category Filter Test Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_demo"
        )
        db.add(schema)
        await db.commit()
        await db.refresh(schema)
        ...
        template1 = C2PAAssertionTemplate(
            name=f"News Template {uuid.uuid4().hex[:8]}", schema_id=schema.id, template_data={}, category="news", organization_id="org_demo"
        )
        template2 = C2PAAssertionTemplate(
            name=f"Legal Template {uuid.uuid4().hex[:8]}", schema_id=schema.id, template_data={}, category="legal", organization_id="org_demo"
        )
        db.add_all([template1, template2])
        await db.commit()

        response = await client.get("/api/v1/enterprise/c2pa/templates?category=news", headers=auth_headers)
    ...
    async def test_get_template(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        schema = C2PASchema(
            name="Get Template Test Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_demo"
        )
        db.add(schema)
        await db.commit()
        await db.refresh(schema)

        template = C2PAAssertionTemplate(name="Get Template", schema_id=schema.id, template_data={}, category="news", organization_id="org_demo")
        db.add(template)
        await db.commit()
        await db.refresh(template)

        response = await client.get(f"/api/v1/enterprise/c2pa/templates/{template.id}", headers=auth_headers)
    ...
    async def test_update_template(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        schema = C2PASchema(
            name="Update Template Test Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_demo"
        )
        db.add(schema)
        await db.commit()
        await db.refresh(schema)
        ...
        update_data = {"description": "Updated description", "category": "legal"}

        response = await client.put(f"/api/v1/enterprise/c2pa/templates/{template.id}", json=update_data, headers=auth_headers)
    ...
    async def test_delete_template(self, client: AsyncClient, auth_headers: dict, db: AsyncSession):
        ...
        schema = C2PASchema(
            name="Delete Template Test Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_demo"
        )
        db.add(schema)
        await db.commit()
        await db.refresh(schema)

        template = C2PAAssertionTemplate(name="Delete Template", schema_id=schema.id, template_data={}, category="news", organization_id="org_demo")
        db.add(template)
        await db.commit()
        await db.refresh(template)

        response = await client.delete(f"/api/v1/enterprise/c2pa/templates/{template.id}", headers=auth_headers)
