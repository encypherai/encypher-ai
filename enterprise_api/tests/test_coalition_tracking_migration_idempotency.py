...
MIGRATION_PATH = Path(__file__).resolve().parents[1] / "alembic" / "versions" / "add_coalition_tracking.py"


@pytest.fixture
def migration_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location("add_coalition_tracking", MIGRATION_PATH)
...
def test_upgrade_skips_existing_coalition_artifacts(monkeypatch: pytest.MonkeyPatch, migration_module: ModuleType) -> None:
    monkeypatch.setattr(migration_module.op, "get_bind", lambda: object())
    monkeypatch.setattr(
        migration_module.sa,
        "inspect",
        lambda _bind: _Inspector(
            tables={"coalition_content_stats", "coalition_earnings", "coalition_payouts"},
            indexes={
                "coalition_content_stats": {"ix_coalition_content_stats_org"},
                "coalition_earnings": {
                ...
                "coalition_payouts": {"ix_coalition_payouts_org", "ix_coalition_payouts_status"},
            },
            unique_constraints={"coalition_content_stats": {"uq_coalition_content_stats_org_period"}},
        ),
    )
