...
class TestMerkleRootCRUD:
    ...
    async def test_get_merkle_roots_by_document(self, db):
        ...
        roots = await merkle_crud.get_merkle_roots_by_document(db, unique_doc_id)

        assert len(roots) == 2

        # Test filtering by level
        sentence_roots = await merkle_crud.get_merkle_roots_by_document(db, unique_doc_id, segmentation_level="sentence")
...
class TestMerkleSubhashCRUD:
    ...
    async def test_find_subhashes_by_hash(self, db):
        ...
        await merkle_crud.create_merkle_subhash(
            db=db, hash_value=target_hash, root_id=root.id, node_type="leaf", depth_level=1, position_index=0, text_content="Find this sentence."
        )
    ...
    async def test_batch_find_subhashes(self, db):
        ...
        for i, hash_val in enumerate(hashes):
            await merkle_crud.create_merkle_subhash(
                db=db, hash_value=hash_val, root_id=root.id, node_type="leaf", depth_level=2, position_index=i, text_content=f"Sentence {i}"
            )

        # Batch find
        results = await merkle_crud.batch_find_subhashes(db, [hashes[0], hashes[2], "nonexistent"])
...
class TestMerkleProofCacheCRUD:
    ...
    async def test_create_and_get_cached_proof(self, db):
        ...
        proof_path = [{"hash": "sibling1" + "0" * 56, "position": "right"}, {"hash": "sibling2" + "0" * 56, "position": "left"}]

        cached = await merkle_crud.create_proof_cache(db=db, leaf_hash="target" + "0" * 58, root_id=root.id, proof_path=proof_path, ttl_hours=24)

        assert cached.id is not None

        # Retrieve the cached proof
        retrieved = await merkle_crud.get_cached_proof(db, "target" + "0" * 58, root.id)
    ...
    async def test_expired_proof_not_returned(self, db):
        ...
        retrieved = await merkle_crud.get_cached_proof(db, "expired" + "0" * 57, root.id)
...
class TestAttributionReportCRUD:
    ...
    async def test_create_attribution_report(self, db):
        ...
        source_docs = [
            {"document_id": "source_1", "matched_segments": 10, "match_percentage": 50.0},
            {"document_id": "source_2", "matched_segments": 5, "match_percentage": 25.0},
        ]
    ...
    async def test_get_attribution_reports_by_organization(self, db):
        ...
        for i in range(3):
            await merkle_crud.create_attribution_report(
                db=db, organization_id=test_org_id, total_segments=10, matched_segments=i, source_documents=[]
            )

        reports = await merkle_crud.get_attribution_reports_by_organization(db, test_org_id)
