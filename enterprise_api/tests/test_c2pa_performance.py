...
class TestC2PAValidatorPerformance:
    ...
    def test_batch_validation_performance(self):
        ...
        assertions = [
            ...
            {"label": "c2pa.claim_review.v1", "data": {"claim_reviewed": "Test claim", "rating": "True"}},
        ]
    ...
    def test_complex_schema_validation_performance(self):
        ...
        data = {
            ...
            "author": {"name": "FactCheck Organization", "url": "https://factcheck.org", "logo": "https://factcheck.org/logo.png"},
            "date_published": "2025-11-03T14:00:00Z",
            "url": "https://factcheck.org/review/12345",
            "item_reviewed": {"type": "Claim", "appearance": {"url": "https://example.com/original"}},
        }
            ...
            schemas.append(
                {
                    ...
                    "schema": {
                        "type": "object",
                        "properties": {"field1": {"type": "string"}, "field2": {"type": "number"}, "field3": {"type": "boolean"}},
                        "required": ["field1"],
                    },
                }
            )
...
class TestC2PAAPILoadTests:
    ...
    async def test_schema_list_pagination_performance(self, client: AsyncClient, auth_headers: dict, db):
        ...
        for page_size in page_sizes:
            ...
            response = await client.get(f"/api/v1/enterprise/c2pa/schemas?page=1&page_size={page_size}", headers=auth_headers)
    ...
    async def test_embedding_with_custom_assertions_performance(self, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.post("/api/v1/enterprise/embeddings/encode-with-embeddings", json=request_data, headers=auth_headers)
...
class TestC2PAMemoryUsage:
    ...
    def test_schema_cache_memory_efficiency(self):
        ...
        for i in range(100):
            schema = {"type": "object", "properties": {f"field{j}": {"type": "string"} for j in range(10)}}
    ...
    def test_large_assertion_validation(self):
        ...
        start = time.perf_counter()
        is_valid, errors, warnings = validator.validate_assertion("c2pa.location.v1", large_data)
        end = time.perf_counter()
...
class TestC2PAStressTests:
    ...
    async def test_rapid_fire_validations(self, client: AsyncClient, auth_headers: dict):
        ...
        async def send_validation():
            return await client.post(
                ...
                json={"label": "c2pa.location.v1", "data": {"latitude": 37.7749, "longitude": -122.4194}},
    ...
    def test_validator_thread_safety(self):
        ...
        def validate_in_thread(thread_id: int):
            results = []
            for i in range(10):
                is_valid, errors, warnings = validator.validate_assertion("c2pa.location.v1", {"latitude": 37.7749, "longitude": -122.4194})
