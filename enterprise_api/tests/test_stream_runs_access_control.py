...
async def test_stream_runs_requires_auth(async_client: AsyncClient) -> None:
    ...
    try:
        logger.info("step=request unauth GET /api/v1/stream/runs/{run_id}")
        resp = await async_client.get(f"/api/v1/stream/runs/{run_id}")
...
async def test_stream_events_requires_auth(async_client: AsyncClient) -> None:
    logger.info("step=request unauth GET /api/v1/stream/events initial_only=true")
    resp = await async_client.get(
        "/api/v1/stream/events",
        params={"session_id": "session_test_requires_auth", "initial_only": True},
    )
    logger.info("step=response status=%s", resp.status_code)
    assert resp.status_code == 401
...
async def test_stream_events_allows_authenticated_stream(
    ...
    logger.info("step=request auth GET /api/v1/stream/events initial_only=true")
    resp = await async_client.get(
        "/api/v1/stream/events",
        params={"session_id": "session_test_allows_auth", "initial_only": True},
...
async def test_stream_runs_requires_matching_org(
    ...
    try:
        logger.info("step=request allowed org GET /api/v1/stream/runs/{run_id}")
        allowed = await async_client.get(
            f"/api/v1/stream/runs/{run_id}",
            headers=business_auth_headers,
        )
        ...
        logger.info("step=request denied org GET /api/v1/stream/runs/{run_id}")
