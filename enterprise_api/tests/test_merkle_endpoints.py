...
class TestDocumentEncodeEndpoint:
    ...
    async def test_encode_document_simple(self, async_client: AsyncClient, business_auth_headers: dict):
        ...
        response = await async_client.post("/api/v1/enterprise/merkle/encode", json=request_data, headers=business_auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_encode_document_with_fuzzy_indexing(
        self,
        async_client: AsyncClient,
        enterprise_auth_headers: dict,
    ):
        """Test encoding with fuzzy fingerprint indexing enabled."""
        doc_id = f"test_doc_{uuid.uuid4().hex[:8]}"
        request_data = {
            "document_id": doc_id,
            "text": "First sentence. Second sentence. Third sentence.",
            "segmentation_levels": ["sentence"],
            "include_words": False,
            "fuzzy_fingerprint": {"enabled": True, "levels": ["sentence"]},
        }

        fuzzy_result = {"indexed_segments": 3, "levels": {"sentence": 3}}
        with patch(
            "app.services.fuzzy_fingerprint_service.fuzzy_fingerprint_service.index_document",
        ...
        assert response.status_code == 201
        data = response.json()
        assert data["fuzzy_index"]["indexed_segments"] == 3
    ...
    async def test_encode_document_invalid_level(self, async_client: AsyncClient, business_auth_headers: dict):
        ...
        request_data = {"document_id": "test_doc_003", "text": "Test text.", "segmentation_levels": ["invalid_level"]}
    ...
    async def test_encode_document_empty_text(self, async_client: AsyncClient, business_auth_headers: dict):
        ...
        request_data = {"document_id": "test_doc_004", "text": "", "segmentation_levels": ["sentence"]}
...
class TestSourceAttributionEndpoint:
    ...
    async def test_find_sources_no_matches(self, async_client: AsyncClient, business_auth_headers: dict):
        ...
        response = await async_client.post("/api/v1/enterprise/merkle/attribute", json=request_data, headers=business_auth_headers)
    ...
    async def test_find_sources_invalid_level(self, async_client: AsyncClient, business_auth_headers: dict):
        ...
        request_data = {"text_segment": "Test text.", "segmentation_level": "invalid"}
...
class TestPlagiarismDetectionEndpoint:
    ...
    async def test_detect_plagiarism_no_matches(self, async_client: AsyncClient, business_auth_headers: dict):
        ...
        response = await async_client.post("/api/v1/enterprise/merkle/detect-plagiarism", json=request_data, headers=business_auth_headers)
...
class TestEndpointIntegration:
    ...
    async def test_encode_then_find(self, async_client: AsyncClient, business_auth_headers: dict):
        ...
        encode_request = {"document_id": doc_id, "text": unique_text, "segmentation_levels": ["sentence"]}

        encode_response = await async_client.post("/api/v1/enterprise/merkle/encode", json=encode_request, headers=business_auth_headers)
        ...
        find_request = {"text_segment": unique_text, "segmentation_level": "sentence", "normalize": True}

        find_response = await async_client.post("/api/v1/enterprise/merkle/attribute", json=find_request, headers=business_auth_headers)
