...
class TestDatabaseSetup:
    ...
    async def test_create_c2pa_schema(self, db: AsyncSession):
        ...
        schema = C2PASchema(
            ...
            label=unique_label,
            version="1.0",
            json_schema={"type": "object", "properties": {"field1": {"type": "string"}}},
    ...
    async def test_create_c2pa_template(self, db: AsyncSession):
        ...
        schema = C2PASchema(name="Template Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_business")
        db.add(schema)
        await db.commit()
        await db.refresh(schema)
        ...
        template = C2PAAssertionTemplate(
            ...
            schema_id=schema.id,
            template_data={"label": "c2pa.location.v1", "data": {"latitude": 37.7749, "longitude": -122.4194}},
            organization_id="org_business",
            is_default=False,
            is_active=True,
        )
    ...
    async def test_database_isolation(self, db: AsyncSession):
        ...
        schema = C2PASchema(
            name="Isolation Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_business"
        )
        db.add(schema)
        await db.commit()

        # Verify it was created
        result = await db.execute(select(C2PASchema).where(C2PASchema.label == unique_label))
...
class TestDatabaseTransactions:
    ...
    async def test_commit_persists_data(self, db: AsyncSession):
        ...
        schema = C2PASchema(name="Commit Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_business")
    ...
    async def test_rollback_discards_data(self, db: AsyncSession):
        ...
        schema = C2PASchema(name="Rollback Schema", label=unique_label, version="1.0", json_schema={"type": "object"}, organization_id="org_business")
