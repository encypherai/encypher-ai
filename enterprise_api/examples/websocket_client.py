...
async def test_streaming():
    ...
    uri = f"ws://localhost:8000/api/v1/sign/stream?api_key={api_key}"
    ...
    try:
        async with websockets.connect(uri) as websocket:
            ...
            # Send test chunks
            test_chunks = ["This is the first sentence. ", "Here comes the second sentence. ", "And finally, the third sentence."]

            print(f"\n📤 Sending {len(test_chunks)} chunks...")

            for i, chunk in enumerate(test_chunks):
                # Send chunk
                message = {"type": "chunk", "content": chunk, "chunk_id": f"chunk_{i:03d}"}
...
async def test_rate_limiting():
    ...
    try:
        async with websockets.connect(uri) as websocket:
            ...
            rate_limited = False
            for i in range(100):
                message = {"type": "chunk", "content": f"Chunk {i}. ", "chunk_id": f"chunk_{i:03d}"}
