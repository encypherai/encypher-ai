...
class BatchCommitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, batch_size: int = 100, batch_timeout: float = 1.0, enabled: bool = True):
        super().__init__(app)
        ...
        if self.enabled:
            ...
            logger.info(f"BatchCommitMiddleware initialized: batch_size={batch_size}, timeout={batch_timeout}s")
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        async with self.lock:
            ...
            should_commit = self.request_count >= self.batch_size or time_since_commit >= self.batch_timeout

            if should_commit:
                logger.debug(f"Batch commit triggered: requests={self.request_count}, time={time_since_commit:.2f}s")
                self.request_count = 0
...
class BatchCommitContext:
    ...
    def __init__(self, session: AsyncSession, middleware: BatchCommitMiddleware, force_commit: bool = False):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.force_commit or not self.middleware.enabled:
            ...
            async with self.middleware.lock:
                should_commit = (
                    self.middleware.request_count >= self.middleware.batch_size
                    or time.time() - self.middleware.last_commit_time >= self.middleware.batch_timeout
                )
...
class DeferredCommitSession:
    ...
    def __init__(self, session: AsyncSession, batch_size: int = 100, batch_timeout: float = 1.0):
        self.session = session
        self.batch_size = batch_size
        self.batch_timeout = batch_timeout
    ...
    async def commit(self):
        ...
        if self._pending_commits >= self.batch_size or time_since_commit >= self.batch_timeout:
