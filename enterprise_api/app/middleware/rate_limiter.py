...
class StreamingRateLimiter:
    ...
    def check_connection_rate(self, organization_id: str, tier: str) -> tuple[bool, Optional[str]]:
        ...
        # Cleanup old entries
        self.connection_attempts[organization_id] = self._cleanup_old_entries(self.connection_attempts[organization_id], window_seconds)

        # Count connections in window
        connection_count = sum(count for _, count in self.connection_attempts[organization_id])

        if connection_count >= max_connections:
            logger.warning(f"Connection rate limit exceeded for org {organization_id}: {connection_count}/{max_connections} in {window_seconds}s")
    ...
    def check_chunk_rate(self, session_id: str, tier: str) -> tuple[bool, Optional[str]]:
        ...
        # Cleanup old entries
        self.chunk_attempts[session_id] = self._cleanup_old_entries(self.chunk_attempts[session_id], window_seconds)

        # Count chunks in window
        chunk_count = sum(count for _, count in self.chunk_attempts[session_id])

        if chunk_count >= max_chunks:
            logger.warning(f"Chunk rate limit exceeded for session {session_id}: {chunk_count}/{max_chunks} in {window_seconds}s")
    ...
    def get_stats(self, organization_id: Optional[str] = None, session_id: Optional[str] = None) -> Dict:
        ...
        if organization_id:
            # Cleanup and count
            self.connection_attempts[organization_id] = self._cleanup_old_entries(self.connection_attempts[organization_id], 60)
            connection_count = sum(count for _, count in self.connection_attempts[organization_id])
            stats["connections_last_minute"] = connection_count

        if session_id:
            # Cleanup and count
            self.chunk_attempts[session_id] = self._cleanup_old_entries(self.chunk_attempts[session_id], 1)
            chunk_count = sum(count for _, count in self.chunk_attempts[session_id])
