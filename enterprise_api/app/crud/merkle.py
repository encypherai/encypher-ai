...
async def get_merkle_roots_by_document(
    db: AsyncSession,
    document_id: str,
...
async def bulk_create_merkle_subhashes(
    db: AsyncSession,
    subhashes: List[Dict[str, Any]]
) -> int:
    """
...
async def find_subhashes_by_root(
    db: AsyncSession,
    root_id: UUID,
    node_type: Optional[str] = None
) -> List[MerkleSubhash]:
...
async def batch_find_subhashes(
    db: AsyncSession,
    hash_values: List[str],
    node_type: Optional[str] = "leaf"
) -> Dict[str, List[MerkleSubhash]]:
    """
