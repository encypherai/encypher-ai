...
class ConnectionManager:
    ...
    async def connect(self, session_id: str, websocket: WebSocket, organization_id: str, metadata: Optional[Dict] = None) -> None:
        """
        ...
        async with self._lock:
            ...
            if org_conn_count >= self.max_connections_per_org:
                raise ConnectionError(f"Maximum connections exceeded for organization {organization_id}. Limit: {self.max_connections_per_org}")
            ...
            logger.info(f"WebSocket connected: session={session_id}, org={organization_id}, total_connections={len(self.active_connections)}")
    ...
    async def disconnect(self, session_id: str) -> None:
        ...
        async with self._lock:
            if session_id in self.active_connections:
                ...
                logger.info(f"WebSocket disconnected: session={session_id}, remaining_connections={len(self.active_connections)}")
