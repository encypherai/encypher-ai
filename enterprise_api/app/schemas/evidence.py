...
class EvidenceGenerateRequest(BaseModel):
    ...
    document_id: Optional[str] = Field(default=None, description="Optional known document ID to verify against")
    include_merkle_proof: bool = Field(default=True, description="Include Merkle proof in evidence package")
    include_signature_chain: bool = Field(default=True, description="Include full signature verification chain")
    include_timestamp_proof: bool = Field(default=True, description="Include timestamp verification")
    export_format: str = Field(default="json", description="Export format: json, pdf, or both")
...
class EvidencePackage(BaseModel):
    ...
    signature_verification: Optional[SignatureVerification] = Field(None, description="Signature verification details")

    # Content matches
    content_matches: List[ContentMatch] = Field(default_factory=list, description="List of matched content segments")
...
class EvidenceListResponse(BaseModel):
    ...
    evidence_packages: List[EvidencePackage] = Field(default_factory=list, description="List of evidence packages")
