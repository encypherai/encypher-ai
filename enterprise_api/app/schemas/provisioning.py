...
class AutoProvisionRequest(BaseModel):
    ...
    source: str = Field(..., description="Source of the provisioning request", examples=["wordpress"])
    source_metadata: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Additional metadata from the source",
        examples=[{"plugin_version": "1.2.3", "site_url": "https://example.com", "wordpress_version": "6.4"}],
    )
    tier: Optional[str] = Field(default="free", description="Initial tier (free/professional/enterprise)", examples=["free"])
    auto_activate: bool = Field(default=True, description="Whether to automatically activate the organization")
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "source": "wordpress",
                "source_metadata": {"plugin_version": "1.2.3", "site_url": "https://example.com"},
                "tier": "free",
                "auto_activate": True,
            }
        }
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "tier": "free",
                "features_enabled": {"merkle_trees": False, "bulk_operations": False, "advanced_analytics": False},
                "quota_limits": {"api_calls_per_month": 1000, "merkle_encoding_per_month": 0},
                "next_steps": {
...
class APIKeyCreateRequest(BaseModel):
    ...
    scopes: Optional[list[str]] = Field(
        ...
        examples=[["merkle:read", "merkle:write"]],
    )
...
class UserAccountCreateRequest(BaseModel):
    ...
    full_name: Optional[str] = Field(None, description="User's full name", max_length=255, examples=["John Doe"])
    organization_id: Optional[str] = Field(None, description="Organization to associate with (creates new if not provided)")
    role: Optional[str] = Field(default="member", description="User role (owner/admin/member)", examples=["member"])
