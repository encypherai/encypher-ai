...
class FingerprintEncodeRequest(BaseModel):
    ...
    fingerprint_key: Optional[str] = Field(default=None, description="Optional custom fingerprint key (auto-generated if not provided)")
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="Optional metadata to associate with fingerprint")
...
class FingerprintDetectRequest(BaseModel):
    ...
    fingerprint_key: Optional[str] = Field(default=None, description="Fingerprint key to search for (searches all if not provided)")
    ...
    return_positions: bool = Field(default=False, description="Return positions of detected markers")
...
class FingerprintMatch(BaseModel):
    ...
    marker_positions: Optional[List[int]] = Field(default=None, description="Positions of detected markers (if requested)")
...
class FingerprintDetectResponse(BaseModel):
    ...
    matches: List[FingerprintMatch] = Field(default_factory=list, description="List of fingerprint matches")
    best_match: Optional[FingerprintMatch] = Field(None, description="Best matching fingerprint (highest confidence)")
    processing_time_ms: float = Field(..., description="Processing time in milliseconds")
    message: str = Field(..., description="Status message")
