...
from app.schemas.fuzzy_fingerprint import FuzzyFingerprintConfig
...
class DocumentEncodeRequest(BaseModel):
    ...
    document_id: str = Field(
        ...
        min_length=1,
        max_length=255,
        examples=["doc_2024_article_001"],
    )
    text: str = Field(
        ...,
        description="Document text content to encode",
        min_length=1,
        examples=["This is the first sentence. This is the second sentence."],
    )
    segmentation_levels: List[str] = Field(
        default=["sentence"],
        description="Segmentation levels to encode (word/sentence/paragraph/section)",
        examples=[["sentence", "paragraph"]],
    )
    include_words: bool = Field(default=False, description="Whether to include word-level segmentation")
    metadata: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Optional document metadata (title, author, etc.)",
        examples=[{"title": "My Document", "author": "John Doe", "date": "2024-10-28"}],
    )
    fuzzy_fingerprint: Optional[FuzzyFingerprintConfig] = Field(
        default=None,
        description="Optional fuzzy fingerprint indexing configuration",
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "include_words": False,
                "metadata": {"title": "Test Article", "author": "Jane Smith", "date": "2024-10-28"},
            }
        }
...
class DocumentEncodeResponse(BaseModel):
    ...
    total_segments: Dict[str, int] = Field(..., description="Number of segments at each level")
    processing_time_ms: float = Field(..., description="Processing time in milliseconds")
    fuzzy_index: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Fuzzy fingerprint indexing summary, if enabled",
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "total_segments": {"sentence": 32, "paragraph": 8},
...
class SourceAttributionRequest(BaseModel):
    ...
    text_segment: str = Field(
        ...
        min_length=1,
        examples=["This is a sentence to find."],
    )
    ...
    normalize: bool = Field(default=True, description="Whether to normalize text before hashing")
    include_proof: bool = Field(default=False, description="Whether to include Merkle proof in response")
...
class PlagiarismDetectionRequest(BaseModel):
    ...
    target_text: str = Field(..., description="Text to check for plagiarism", min_length=1)
    target_document_id: Optional[str] = Field(None, description="Optional identifier for the target document")
    segmentation_level: str = Field(default="sentence", description="Segmentation level to analyze")
    include_heat_map: bool = Field(default=True, description="Whether to generate heat map visualization data")
    min_match_percentage: float = Field(default=0.0, description="Minimum match percentage to include in results", ge=0, le=100)
