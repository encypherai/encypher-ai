...
class MultiSourceLookupRequest(BaseModel):
    ...
    include_all_sources: bool = Field(
        default=True,
        description="Return all matching sources (not just first)"
    )
    order_by: str = Field(
        default="chronological",
        description="Ordering: chronological (earliest first), authority, or confidence"
    )
    include_authority_score: bool = Field(
        default=False,
        description="Include authority ranking scores (Enterprise feature)"
    )
...
class MultiSourceLookupResponse(BaseModel):
    ...
    original_source: Optional[SourceRecord] = Field(
        None,
        description="The original/earliest source"
    )
...
class AuthorityConfig(BaseModel):
    ...
    organization_weights: Dict[str, float] = Field(
        default_factory=dict,
        description="Custom weights for specific organizations (0-1)"
    )
    prefer_verified: bool = Field(
        default=True,
        description="Prefer verified/trusted organizations"
    )
