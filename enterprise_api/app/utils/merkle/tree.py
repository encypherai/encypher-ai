...
class MerkleTree:
    ...
    def __init__(self, segments: List[str], segmentation_level: str = "sentence", metadata: Optional[Dict[str, Any]] = None):
        """
        ...
        logger.debug(f"Built Merkle tree: {self.total_leaves} leaves, depth {self.tree_depth}, root hash {self.root.hash[:8]}...")
    ...
    def _build_tree(self, segments: List[str]) -> MerkleNode:
        ...
        for i, segment in enumerate(segments):
            ...
            node = MerkleNode(
                hash=segment_hash,
                content=segment,
                metadata={"index": i, "level": self.segmentation_level, "length": len(segment)},
        ...
        while len(current_level) > 1:
            ...
            for i in range(0, len(current_level), 2):
                ...
                parent = MerkleNode(hash=parent_hash, left=left, right=right, depth=depth, position=i // 2)
    ...
    def __repr__(self) -> str:
        ...
        return f"MerkleTree(leaves={self.total_leaves}, depth={self.tree_depth}, root={self.root.hash[:8]}...)"
