...
class MerkleProof:
    ...
    def __repr__(self) -> str:
        ...
        return f"MerkleProof(target={self.target_hash[:8]}..., root={self.root_hash[:8]}..., steps={len(self.proof_path)}, verified={self.verified})"
...
def generate_proof(tree_root: MerkleNode, target_hash: str) -> Optional[MerkleProof]:
    ...
    def find_path(node: MerkleNode) -> bool:
        ...
        if node.right and node.right != node.left and find_path(node.right):
            ...
            sibling = node.left or node.right
            proof_path.append(ProofStep(hash=sibling.hash, position="left"))
            return True
    ...
    # Create proof
    proof = MerkleProof(target_hash=target_hash, root_hash=tree_root.hash, proof_path=proof_path)

    logger.debug(f"Generated proof: {len(proof_path)} steps, target={target_hash[:8]}..., root={tree_root.hash[:8]}...")
...
def verify_proof(proof: MerkleProof) -> bool:
    ...
    if is_valid:
        ...
        logger.warning(f"Proof verification failed: expected {proof.root_hash[:8]}..., got {current_hash[:8]}...")
...
def batch_generate_proofs(tree_root: MerkleNode, target_hashes: List[str]) -> List[Optional[MerkleProof]]:
    """
