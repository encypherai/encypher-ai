...
class C2PAVerifier:
    ...
    async def verify_manifest_url(self, manifest_url: str) -> C2PAVerificationResult:
        ...
        try:
            ...
            return C2PAVerificationResult(valid=False, manifest_url=manifest_url, errors=[f"Failed to fetch manifest: {str(e)}"])
        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP error fetching C2PA manifest from {manifest_url}: {e}")
            return C2PAVerificationResult(valid=False, manifest_url=manifest_url, errors=[f"HTTP error: {str(e)}"])
        except json.JSONDecodeError as e:
            logger.error(f"Invalid JSON in C2PA manifest: {e}")
            return C2PAVerificationResult(valid=False, manifest_url=manifest_url, errors=["Invalid JSON in manifest"])
        except Exception as e:
            logger.error(f"Unexpected error verifying C2PA manifest: {e}", exc_info=True)
            return C2PAVerificationResult(valid=False, manifest_url=manifest_url, errors=[f"Verification error: {str(e)}"])
    ...
    def _verify_manifest_data(
        self, manifest_data: Dict[str, Any], manifest_url: Optional[str] = None, manifest_hash: Optional[str] = None
