...
class HierarchicalSegmenter:
    ...
    def build_hierarchy(self) -> Dict[str, List[HierarchicalSegment]]:
        ...
        """
        hierarchy: Dict[str, List[HierarchicalSegment]] = {"sentence": [], "paragraph": [], "section": []}

        # Create sentence segments
        for i, sentence in enumerate(self.sentences):
            hierarchy["sentence"].append(HierarchicalSegment(text=sentence, level="sentence", index=i, metadata={"length": len(sentence)}))
...
def build_hierarchical_structure(text: str) -> Dict[str, List[str]]:
    ...
    return {"sentences": segmenter.sentences, "paragraphs": segmenter.paragraphs, "sections": segmenter.sections}
