...
def segment_sections(text: str, min_length: int = 50) -> List[str]:
    ...
    for line in lines:
        ...
        is_all_caps_header = (
            len(stripped) > 0 and len(stripped) < 100 and stripped.isupper() and not stripped.startswith(("HTTP", "URL", "API", "ID"))
        )

        is_section_break = is_markdown_header or is_horizontal_rule or is_numbered_section or is_all_caps_header
    ...
    for line in lines:
        ...
        if header_match:
            ...
            if current_title is not None:
                sections.append({"title": current_title, "content": "\n".join(current_content).strip(), "level": current_level})
