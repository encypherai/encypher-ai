...
class EvidenceService:
    ...
    async def generate_evidence(
        ...
        matches, source_info = await self._lookup_matches(
            db, segment_hashes, segments, document_id
        )
        ...
        if include_merkle_proof and matches and source_info.get("merkle_root_id"):
            merkle_proof, merkle_root_hash, proof_valid = await self._generate_merkle_proof(
                db, source_info["merkle_root_id"], matches[0].leaf_index
            )
        ...
        if include_signature_chain and source_info.get("signer_id"):
            sig_verification = await self._verify_signature_chain(
                db, source_info
            )
        ...
        logger.info(
            f"Generated evidence package {evidence_id}: "
            f"attribution_found={attribution_found}, confidence={attribution_confidence:.2f}"
        )
    ...
    async def _lookup_matches(
        ...
        for idx, (seg_hash, segment) in enumerate(zip(segment_hashes, segments)):
            stmt = select(ContentReference).where(
                ContentReference.leaf_hash == seg_hash
            )
            if document_id:
    ...
    async def _generate_merkle_proof(
        ...
        stmt = select(ContentReference).where(
            ContentReference.merkle_root_id == merkle_root_id
        ).order_by(ContentReference.leaf_index)
        result = await db.execute(stmt)
        refs = result.scalars().all()
        
        if not refs:
            return [], root.root_hash, False
        
        # Build proof path
        hashes = [ref.leaf_hash for ref in refs]
            ...
            for i in range(0, len(hashes), 2):
                ...
                if i == current_index or i + 1 == current_index:
                    if current_index % 2 == 0:
                        ...
                        proof_items.append(MerkleProofItem(
                            hash=right,
                            position="right",
                            level=level,
                        ))
        ...
        # Verify the proof
        is_valid = len(hashes) == 1 and hashes[0] == root.root_hash
        
        return proof_items, root.root_hash, is_valid
