...
from app.utils.merkle.proof import ProofStep
...
class MerkleService:
    ...
    async def find_sources(
        db: AsyncSession, text_segment: str, segmentation_level: str = "sentence", normalize: bool = True
    ...
    async def generate_attribution_report(
        ...
        for idx, segment in enumerate(segments):
            sources = await MerkleService.find_sources(db=db, text_segment=segment, segmentation_level=segmentation_level)
            ...
            if sources:
                ...
                for subhash, root in sources:
                    doc_id = cast(str, root.document_id)
                    if doc_id not in source_matches:
                        source_matches[doc_id] = {
                            "document_id": doc_id,
                            "organization_id": cast(str, root.organization_id),
                            "segmentation_level": cast(str, root.segmentation_level),
                            "matched_segments": 0,
                ...
                heat_map_positions.append({"index": idx, "segment": segment[:100], "matched": False, "source_count": 0})
        ...
        report = await merkle_crud.create_attribution_report(
            ...
            report_metadata={"segmentation_level": segmentation_level, "text_length": len(target_text)},
        )

        logger.info(
            f"Generated attribution report: {matched_segments}/{total_segments} segments matched, {len(source_documents)} source documents found"
        )
    ...
    async def verify_segment_in_document(
        db: AsyncSession, text_segment: str, document_id: str, segmentation_level: str = "sentence"
        ...
        roots = await merkle_crud.get_merkle_roots_by_document(db=db, document_id=document_id, segmentation_level=segmentation_level)
        ...
        # Check cache first
        cached_proof = await merkle_crud.get_cached_proof(db=db, leaf_hash=segment_hash, root_id=root.root_id)

        if cached_proof:
            # Reconstruct proof from cache
            raw_steps: List[Any] = cast(List[Any], cached_proof.proof_path or [])
            proof_steps = [ProofStep.from_dict(step) for step in raw_steps if isinstance(step, dict)]
            proof = MerkleProof(
                target_hash=cast(str, cached_proof.leaf_hash),
        ...
        subhashes = await merkle_crud.find_subhashes_by_root(db=db, root_id=root.root_id)
