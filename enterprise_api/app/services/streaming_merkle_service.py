...
class StreamingMerkleService:
    ...
    async def start_session(
        ...
        logger.info(
            f"Started streaming Merkle session {session_id} for document {document_id} "
            f"(org: {organization_id}, level: {segmentation_level})"
        )
    ...
    async def add_segment(
        ...
        logger.debug(
            f"Added segment {len(session.segment_hashes)-1} to session {session_id}: "
            f"hash={segment_hash[:16]}..."
        )
    ...
    async def finalize_session(
        ...
        if embed_manifest and embedding_service and session.segments:
            try:
                ...
                logger.info(
                    f"Embedded manifest for finalized session {session_id}: "
                    f"mode={manifest_mode}, segments={len(session.segments)}"
                )
    ...
    async def cleanup_expired_sessions(self) -> int:
        ...
        to_remove = [
            sid for sid, session in self._sessions.items()
            if session.status in ("expired", "finalized") and session.last_activity < one_hour_ago
        ]
