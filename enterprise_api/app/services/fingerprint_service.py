...
class FingerprintService:
    ...
    async def encode_fingerprint(
        ...
        logger.info(
            f"Encoded fingerprint {fingerprint_id} for doc {document_id}: "
            f"{len(positions)} markers at density {density}"
        )
    ...
    async def detect_fingerprint(
        ...
        fingerprints_to_check = self._fingerprints.values()
        if fingerprint_key:
            # Filter to specific key
            fingerprints_to_check = [
                fp for fp in fingerprints_to_check
                if fp["key"] == fingerprint_key
            ]
        ...
        if detected:
            logger.info(
                f"Detected {len(matches)} fingerprint(s) in text, "
                f"best confidence: {matches[0].confidence:.2f}"
            )
