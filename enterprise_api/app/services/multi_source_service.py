...
class MultiSourceService:
    ...
    async def lookup_sources(
        ...
        stmt = select(ContentReference).where(
            ContentReference.leaf_hash == query_hash
        )
        ...
        for idx, ref in enumerate(refs):
            ...
            org_name = await self._get_organization_name(db, ref.organization_id)
            ...
            if include_authority_score:
                authority_score = await self._calculate_authority_score(
                    db, ref.organization_id, ref.created_at
                )
            ...
            previous_id = refs[idx - 1].document_id if idx > 0 else None
            next_id = refs[idx + 1].document_id if idx < len(refs) - 1 else None
            ...
            source = SourceRecord(
                ...
                signed_at=ref.created_at,  # Use created_at as signed_at
        ...
        logger.info(
            f"Multi-source lookup for hash {query_hash[:16]}...: "
