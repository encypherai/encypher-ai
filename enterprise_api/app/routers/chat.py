...
@router.post("/stream/chat/openai-compatible")
async def openai_compatible_chat(
    ...
    if not request.sign_response:
        raise HTTPException(
            status_code=400,
            detail="This endpoint requires sign_response=true. "
                   "Use standard OpenAI API for unsigned responses."
        )
    
    if request.stream:
        # Return streaming response
        return StreamingResponse(
            _stream_chat_completion(request, organization),
