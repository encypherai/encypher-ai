...
async def seed_schemas(session: AsyncSession):
    ...
    for schema_data in schemas:
        ...
        stmt = select(C2PASchema).where(
            C2PASchema.namespace == schema_data["namespace"], C2PASchema.label == schema_data["label"], C2PASchema.version == schema_data["version"]
        )
...
async def seed_templates(session: AsyncSession):
    ...
    for template_data in templates:
        ...
        stmt = select(C2PAAssertionTemplate).where(
            C2PAAssertionTemplate.name == template_data["name"], C2PAAssertionTemplate.category == template_data["category"]
        )
        result = await session.execute(stmt)
        existing = result.scalar_one_or_none()
