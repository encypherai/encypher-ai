...
$logPath = Join-Path $logsDir ("c2pa_{0}_{1}.log" -f $Concurrency,$ts)

$cpu = Start-CpuLog -Name ("c2pa_{0}" -f $Concurrency)
try {
  Push-Location $repoRoot
  & uv run python tools/large_dataset_sign_verify.py --mode c2pa --limit $Limit --concurrency $Concurrency --no-verify --base-url $BaseUrl 2>&1 |
...
$summary = Select-String -Path $logPath -Pattern 'Signing Summary','Signing Performance','Files:','Total Time:','P50 per file:','P95 per file:' | ForEach-Object { $_.Line }

Write-Host "`n=== C2PA @ $Concurrency Summary ===" -ForegroundColor Cyan
