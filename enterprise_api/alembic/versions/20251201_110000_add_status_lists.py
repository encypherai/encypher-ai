...
def upgrade() -> None:
    ...
    op.create_index(
        ...
        'status_list_metadata',
        ['organization_id', 'list_index'],
        unique=True
    )
    op.create_index(
        'idx_status_meta_stale',
        'status_list_metadata',
        ['organization_id', 'last_generated_at']
    )
    ...
    op.add_column(
        'content_references',
        sa.Column('status_bit_index', sa.Integer(), nullable=True)
    )
    op.add_column(
        'content_references',
        sa.Column('status_list_url', sa.String(500), nullable=True)
    )
