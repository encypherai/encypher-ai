...
def upgrade() -> None:
    ...
    if not _has_table("coalition_content_stats"):
        op.create_table(
    ...
    if not _has_table("coalition_earnings"):
        op.create_table(
            "coalition_earnings",
            sa.Column("id", sa.String(32), primary_key=True),
            sa.Column("organization_id", sa.String(64), nullable=False),
    ...
    if not _has_index("coalition_earnings", "ix_coalition_earnings_org"):
        op.create_index("ix_coalition_earnings_org", "coalition_earnings", ["organization_id"])
    if not _has_index("coalition_earnings", "ix_coalition_earnings_deal"):
        op.create_index("ix_coalition_earnings_deal", "coalition_earnings", ["deal_id"])
    if not _has_index("coalition_earnings", "ix_coalition_earnings_period"):
        op.create_index("ix_coalition_earnings_period", "coalition_earnings", ["period_start", "period_end"])
    if not _has_index("coalition_earnings", "ix_coalition_earnings_status"):
        op.create_index("ix_coalition_earnings_status", "coalition_earnings", ["status"])

    # Create coalition_payouts table
    # Tracks actual payments to publishers
    if not _has_table("coalition_payouts"):
        op.create_table(
            "coalition_payouts",
            sa.Column("id", sa.String(32), primary_key=True),
            sa.Column("organization_id", sa.String(64), nullable=False),
    ...
    if not _has_index("coalition_payouts", "ix_coalition_payouts_org"):
        op.create_index("ix_coalition_payouts_org", "coalition_payouts", ["organization_id"])
    if not _has_index("coalition_payouts", "ix_coalition_payouts_status"):
        op.create_index("ix_coalition_payouts_status", "coalition_payouts", ["status"])
