...
- [ ] 2.1 Create `/auth/verify-email` page in dashboard
  - [ ] 2.1.1 Accept `token` query parameter
  - [ ] 2.1.2 Call auth-service `/api/v1/auth/verify-email` endpoint
  - [ ] 2.1.3 Handle success: auto-login user with returned tokens
  - [ ] 2.1.4 Handle error: show appropriate message
- [ ] 2.2 Update auth-service email template URL if needed
- [ ] 2.3 Implement auto-login after verification
  - [ ] 2.3.1 Use NextAuth `signIn('credentials')` with returned tokens
  - [ ] 2.3.2 Or implement custom session creation
...
- [ ] 4.1 Diagnose the split-brain issue
  - [ ] 4.1.1 key-service creates keys in `api_keys` table with `user_id`
  - [ ] 4.1.2 enterprise-api expects keys joined to `organizations` table
  - [ ] 4.1.3 New users have no organization record in enterprise-api DB
- [ ] 4.2 Design solution (choose one):
  ...
  - **Option D**: Use key-service's `/validate` endpoint which returns synthetic org context
- [ ] 4.3 Implement chosen solution
