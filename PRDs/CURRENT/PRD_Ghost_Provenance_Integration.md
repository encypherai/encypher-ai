...
**Hosted Ghost embedding duplicate-text fix (2026-02-17, TEAM_206).** Fixed Python hosted integration HTML embedding to avoid replacing duplicate text inside skipped Ghost card/code regions (`kg-code-card`, `code`, `pre`, `script`, `style`) before visible content. Added regression test ensuring markers are applied to visible paragraph text when skipped-card text matches exactly. ✅ `uv run pytest enterprise_api/tests/test_ghost_integration.py` (68 passed) + ✅ `uv run ruff check enterprise_api/app/services/ghost_integration.py enterprise_api/tests/test_ghost_integration.py`

**Ghost `source=html` wrapper hardening (2026-02-17, TEAM_206).** Added regression coverage and implementation hardening for html-card wrappers (`<!--kg-card-begin: html-->...<!--kg-card-end: html-->`) so wrapped payloads are excluded from extraction and marker insertion paths in `integrations/ghost-provenance` TypeScript utilities. Added docs-backed edge-case matrix (`integrations/ghost-provenance/EDGE_CASE_MATRIX.md`) and an opt-in local replay harness test (`tests/local-replay.test.ts`) for end-to-end fixture replay against local Ghost + integration service. ✅ `npm test` (69 passed, 1 local replay skipped by default)
