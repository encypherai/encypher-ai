...
## Overview
The admin dashboard currently shows seeded placeholder data and omits real auth-service users (e.g., test@encypher.com). We need to wire the admin stats and user directory to live auth-service data so super admins see accurate user and organization metrics.
...
- Admin user directory includes test@encypher.com and other auth users.
...
- Enforced super admin flag for `erik.svilich@encypher.com` at auth-service startup with ✅ `uv run pytest tests/test_super_admin_startup.py`.
