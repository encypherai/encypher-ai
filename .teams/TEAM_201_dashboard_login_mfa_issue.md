...
- Verified local auth DB record for `test@encypherai.com` has MFA disabled (`totp_enabled = false`).
...
## Verification
- `docker exec encypher-postgres psql -U encypher -d encypher_auth -c "SELECT email, totp_enabled, (totp_secret_encrypted IS NOT NULL) AS has_totp_secret, totp_enabled_at, totp_backup_code_hashes FROM users WHERE email = 'test@encypherai.com';"` ✅
