...
def test_sign_and_verify():
    ...
    sign_payload = {"text": "Hello, world.", "metadata_format": "c2pa", "signer_id": "org_demo"}
    ...
    try:
        with httpx.Client(timeout=30.0) as client:
            sign_response = client.post(f"{ENTERPRISE_API_BASE}/sign", json=sign_payload, headers={"Content-Type": "application/json"})
    ...
    verify_payload = {"text": signed_text}
    ...
    try:
        with httpx.Client(timeout=30.0) as client:
            verify_response = client.post(f"{API_BASE}/verify", json=verify_payload, headers={"Content-Type": "application/json"})
