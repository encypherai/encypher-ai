...
from encypher.core.crypto_utils import (
    generate_key_pair,  # Needed for manual verification check
)
...
class TestUnicodeMetadata:
    ...
    def test_embed_verify_extract_success(
        ...
        extracted_payload, is_valid, extracted_signer_id = (
        ...
        if metadata_format == "basic":
            ...
            assert extracted_payload.get("custom_metadata") == original_payload.get(
                "custom_metadata"
            )
        elif metadata_format == "manifest":
            # Access nested manifest fields
            manifest_payload = extracted_payload.get("manifest", {})
            assert manifest_payload.get("claim_generator") == original_payload.get(
                "claim_generator"
            )
            ...
            assert manifest_payload.get("custom_claims") == original_payload.get(
                "custom_claims"
            )
    ...
    def test_verify_wrong_key(
        ...
        def wrong_key_provider(s_id: str) -> Optional[PublicKeyTypes]:
            if s_id == "signer_1":
                return public_key_signer2  # Return signer 2's key for signer 1's ID
            return None
        ...
        assert extracted_signer_id == signer_id
        assert (
            extracted_payload is not None
        )  # Payload is extracted, but verification fails
    ...
    def test_verify_failure_wrong_key(
        ...
        extracted_payload, is_valid, extracted_signer_id = (
            UnicodeMetadata.verify_and_extract_metadata(
                encoded_text, public_key_provider
            )
        )
    ...
    def test_verify_failure_invalid_signature_format(
        ...
        for i, char in enumerate(encoded_text):
            code_point = ord(char)
            if (
                UnicodeMetadata.VARIATION_SELECTOR_START
                <= code_point
                <= UnicodeMetadata.VARIATION_SELECTOR_END
            ):
                # Replace this variation selector with a different one
                corrupted_text = (
                    encoded_text[:i] + chr(code_point + 1) + encoded_text[i + 1 :]
                )
                break
            elif (
                UnicodeMetadata.VARIATION_SELECTOR_SUPPLEMENT_START
                <= code_point
                <= UnicodeMetadata.VARIATION_SELECTOR_SUPPLEMENT_END
            ):
                # Replace this variation selector with a different one
                corrupted_text = (
                    encoded_text[:i] + chr(code_point + 1) + encoded_text[i + 1 :]
            ...
            corrupted_text = encoded_text + chr(
                UnicodeMetadata.VARIATION_SELECTOR_START
            )
        ...
        extracted_payload, is_valid, extracted_signer_id = (
            UnicodeMetadata.verify_and_extract_metadata(
                corrupted_text, public_key_provider
            )
        )
    ...
    def test_verify_failure_key_mismatch(self, key_pair_1, sample_text):
        ...
        def wrong_key_provider(s_id: str) -> Optional[PrivateKeyTypes]:
            if s_id == "signer_1":
                return private_key  # Return private key for signer 1
            return None  # Original for others

        extracted_payload, is_valid, extracted_signer_id = (
            UnicodeMetadata.verify_and_extract_metadata(
                encoded_text, wrong_key_provider
            )
        )
    ...
    def test_verify_failure_provider_error(self, key_pair_1, sample_text):
        ...
        extracted_payload, is_valid, extracted_signer_id = (
            UnicodeMetadata.verify_and_extract_metadata(encoded_text, error_provider)
        )

        assert not is_valid
        assert extracted_payload is None

    def test_unicode_metadata_extract_metadata(
        self, sample_text, basic_metadata, key_pair_1
    ):
        ...
        assert extracted_metadata.get("custom_metadata") == metadata_to_embed.get(
            "custom_metadata"
        )
    ...
    def test_unicode_metadata_extract_metadata_no_metadata(self, sample_text):
        ...
        assert (
            extracted_metadata is None
        ), "Should return None when no metadata is embedded"

    def test_unicode_metadata_extract_metadata_corrupted(
        self, sample_text, basic_metadata, key_pair_1
    ):
    ...
    def test_unicode_metadata_verify_with_manifest(
        self, manifest_metadata, key_pair_1, sample_text, public_key_provider
    ):
        ...
        # Access nested manifest fields
        manifest_payload = extracted_payload.get("manifest", {})
        assert manifest_payload.get("claim_generator") == manifest_metadata.get(
            "claim_generator"
        )
        ...
        assert manifest_payload.get("custom_claims") == manifest_metadata.get(
            "custom_claims"
        )

    @pytest.mark.skip(
        reason=(
            "Test based on old embed_metadata signature (HMAC), incompatible "
            "with new signature-based method."
        )
    )
