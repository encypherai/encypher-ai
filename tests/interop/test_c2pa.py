...
class TestC2PAInterop(unittest.TestCase):
    ...
    def test_encypher_to_c2pa_conversion(self):
        ...
        # Verify core fields
        self.assertEqual(
            c2pa_result["claim_generator"], self.encypher_manifest["claim_generator"]
        )
        ...
        self.assertEqual(
            c2pa_result["custom_claims"], self.encypher_manifest["custom_claims"]
        )
    ...
    def test_c2pa_to_encypher_conversion(self):
        ...
        # Verify core fields
        self.assertEqual(
            manifest_result["claim_generator"], self.c2pa_dict["claim_generator"]
        )
        ...
        self.assertEqual(
            len(assertions), 2
        )  # 2 regular assertions, 1 ai_assertion assertion
        ...
        self.assertEqual(
            manifest_result["custom_claims"], self.c2pa_dict["custom_claims"]
        )
    ...
    def test_roundtrip_conversion(self):
        ...
        self.assertEqual(
            manifest_result["timestamp"], self.encypher_manifest["timestamp"]
        )

        # Verify assertions (may have different order, so check by assertion type)
        original_assertions = {
            a["label"]: a for a in self.encypher_manifest["assertions"]
        }
        ...
        # Verify custom claims
        self.assertEqual(
            manifest_result["custom_claims"], self.encypher_manifest["custom_claims"]
        )
