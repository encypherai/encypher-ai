...
import { Newspaper, Shield, Loader2, CheckCircle2, FileWarning, Copy, Check } from "lucide-react";
import { cn } from "@/lib/utils";
import { PUBLISHER_ARTICLE, generateDocumentId } from "@/lib/demo-data";
import { encodeContent, EmbeddingInfo } from "@/lib/api";

interface PublisherArticleProps {
...
export default function PublisherArticle({ onContentMarked, markedContent, highlightedSentence }: PublisherArticleProps) {
  ...
  const handleCopySignedText = async () => {
  ...
  const handleMarkContent = async () => {
    ...
    try {
      const fullText = PUBLISHER_ARTICLE.paragraphs.join("\n\n");
      
      const response = await encodeContent({
        document_id: generateDocumentId(),
        text: fullText,
        metadata: {
          title: PUBLISHER_ARTICLE.headline,
          author: PUBLISHER_ARTICLE.byline,
          source: PUBLISHER_ARTICLE.source,
          publisher: PUBLISHER_ARTICLE.publisher,
      ...
      if (response.success && response.embedded_content) {
        ...
        if (response.embeddings && response.embeddings.length > 0) {
          ...
          for (const originalParagraph of PUBLISHER_ARTICLE.paragraphs) {
      ...
      setEmbeddedParagraphs(PUBLISHER_ARTICLE.paragraphs);
      setIsMarked(true);
    } finally {
      setIsMarking(false);
    }
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
      ...
      <div className="bg-slate-800 text-white px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-lg">{PUBLISHER_ARTICLE.source}</div>
            <div className="text-gray-400 text-sm">{PUBLISHER_ARTICLE.publisher}</div>
          </div>
        </div>
        <div className="text-right text-sm text-gray-400">
          {PUBLISHER_ARTICLE.date}
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-3 leading-tight">
          {PUBLISHER_ARTICLE.headline}
        </h1>
        
        {/* Byline */}
        <div className="text-gray-600 text-sm mb-4">
          {PUBLISHER_ARTICLE.byline}
        </div>
        
        {/* Dateline */}
        <div className="text-gray-500 text-sm mb-6 font-medium">
          {PUBLISHER_ARTICLE.dateline} —
        </div>

        {/* Article Body */}
        <div 
          id="publisher-article-body"
          className={cn(
        ...
        >
          {(isMarked && embeddedParagraphs.length > 0 ? embeddedParagraphs : PUBLISHER_ARTICLE.paragraphs).map((paragraph: string, index: number) => {
              ...
              >
                {isMarked ? (
                  ...
                  <span className="relative group">
                    {paragraph.split(/(?<=[.!?])\s+/).map((sentence: string, sIndex: number) => (
                      <span 
                        key={sIndex} 
          ...
          <div className="mt-6 space-y-2">
            ...
            <div className="flex items-center gap-2 text-amber-700 bg-amber-50 px-4 py-3 rounded-lg border border-amber-200">
              ...
              <span className="text-amber-600 text-sm ml-auto">
                Content marked as owned by {PUBLISHER_ARTICLE.source}
              </span>
            </div>
            
            {/* Copy Signed Text Button */}
            <button
              onClick={handleCopySignedText}
              className={cn(
            ...
            >
              {isCopied ? (
                ...
                <>
                  ...
                  Copy Signed Text (for verification)
                </>
              )}
            </button>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-500">
            {isMarked ? (
              ...
              "Standard news article - no provenance embedded"
            )}
          </div>
