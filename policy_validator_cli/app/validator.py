...
def validate_metadata(metadata: Dict[str, Any], policy_rules: List[Dict[str, Any]], source_identifier: str = "input") -> Tuple[bool, List[str]]:
    """
    ...
    for rule in policy_rules:
        ...
        if expected_type:
            ...
            if not type_valid:
                is_valid = False
                errors.append(
                    f"Source '{source_identifier}': Metadata key '{key}' has value '{value}' (type: {type(value).__name__}), which does not match expected type '{expected_type}'."
                )
        ...
        if allowed_values:
            if value not in allowed_values:
                is_valid = False
                errors.append(
                    f"Source '{source_identifier}': Metadata key '{key}' has value '{value}', which is not in the allowed list: {allowed_values}."
                )
